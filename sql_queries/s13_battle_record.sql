...
-- S13 战绩页指标 SQL
-- 项目: tapdb_one_data
-- 目标表: ads_torchlight_s13_battle_record_df
-- 更新频率: 每日全量
-- 产出粒度: account + season（每个账号每赛季一行）
--
-- 指标 → 数据源 对照:
--   01. 累计获得渴瘾物质数量       dwd_torchlight_coin_ri          coin_id='826'
...
--   10. 选择次数最多的手术用具     dwd_torchlight_s13gameplay_plague_di  s13gameplay_plague_finish.card_relic_list  EXPLODE+MODE
--   11. 选择次数最多的药剂         dwd_torchlight_s13gameplay_plague_di  s13gameplay_plague_finish.card_effect_list EXPLODE+MODE
...
cte_level AS (
    ...
    WHERE dt BETWEEN '2026-01-17' AND '${bizdate}'
...
cte_reward_raw AS (
    ...
    WHERE name         = 's13gameplay_level_finish'
      AND dt BETWEEN '2026-01-17' AND '${bizdate}'
...
cte_colored_jar AS (
    SELECT
        account,
        season,
        COUNT(CASE WHEN box_rarity = 7 THEN 1 END) AS colored_jar_cnt
    FROM cte_reward_raw
    GROUP BY account, season
),
...
SELECT
    ...
    COALESCE(cr.craving_substance_cnt,     0) AS craving_substance_cnt,     -- 01 累计获得渴瘾物质数量
    COALESCE(it.perfect_organ_cnt,         0) AS perfect_organ_cnt,         -- 02 累计掉落完美器官数量
    COALESCE(sc.scavenger_kill_cnt,        0) AS scavenger_kill_cnt,        -- 03 累计击杀清道夫数量
    COALESCE(lv.level_open_cnt,            0) AS level_open_cnt,            -- 04 终局玩法开启总次数
    COALESCE(lv.level_score_max,           0) AS level_score_max,           -- 05 终局玩法单局最高分
    COALESCE(lv.level_score_min,           0) AS level_score_min,           -- 06 终局玩法单局最低分
    COALESCE(cj.colored_jar_cnt,           0) AS colored_jar_cnt,           -- 07 累积开启彩色罐子
    COALESCE(pm.max_monster_num,           0) AS max_monster_num,           -- 08 终局玩法养成最多怪物数量
    COALESCE(ps.max_single_monster_score,  0) AS max_single_monster_score,  -- 09 终局玩法养成最高单个怪物分数
    tm.most_used_modifier,                                                   -- 10 选择次数最多的手术用具
    te.most_used_effect,                                                     -- 11 选择次数最多的药剂
    COALESCE(vi.total_vitality,            0) AS total_vitality,            -- 12 赛季内累计获得的总活性
